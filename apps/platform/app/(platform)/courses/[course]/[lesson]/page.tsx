...
export default async function LessonPage({
  ...
  // Written by the sidebar itself; read here so a collapsed rail survives a
  // reload without a flash of the open state. Shared with the tools shell on
  // purpose — someone who collapses the rail means it.
  const collapsed = (await cookies()).get("sidebar_state")?.value === "false";

  return (
    /* The reader is on shadcn's `Sidebar` like every other shell (S16 D3), so
       the package has one sidebar system rather than two. `offcanvas`, not
       `icon`: a rail of lesson titles has no meaningful icon form, and the
       trigger hides it outright — which is also what gives the reader its
       mobile drawer for free, replacing the lazily-loaded one. */
    <SidebarProvider defaultOpen={!collapsed}>
      ...
      <SidebarInset>
        ...
        <main
          className={cn(
            "mx-auto flex w-full max-w-4xl min-w-0 flex-col gap-8 px-4 py-8 sm:px-6 lg:px-10 lg:py-10",
        ...
        >
          ...
          {lesson.type === "project" ? (
            <div className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">
              Project briefs are here to work through on your own. Submitting
              and reviewing them is not part of this release; the MVP is

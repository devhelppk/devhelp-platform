...
- **An intermittent test failure under parallel runs, seen in S8, S10b, and S11.** Always a different test, always one that ends in a certificate insert inside a transaction (`emits exactly one course_completed when the last two lessons complete concurrently`, `records exercise submissions…`, the certificates suite), always passing on a rerun and in isolation. Four sightings across three specs is no longer a coincidence; the shared ingredient is `issueCertificate` running inside the reducer's transaction while another suite holds rows it touches. Worth an hour with `pg_stat_activity` and deliberate lock ordering before it costs someone a red CI run they cannot reproduce.

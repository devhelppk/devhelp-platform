...
1. **The orientation facts are last in the DOM.** `Facts` — industry, size, founded, cities, hires juniors, website — is an `<aside>` after reviews, pay and interviews. On desktop it is a right-hand column and reads acceptably; **on mobile it collapses to the very bottom**, so a phone visitor scrolls past three long sections of opinion before learning what the company does or where it is. The mobile order should be: who they are → what the numbers say → the opinions behind them.

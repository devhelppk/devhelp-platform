...
Status: `done` (see `review.md`, `test.md`). Deviations after implementation and review: badges are evaluated after the read models move (a path badge could not otherwise see the completion that finished it); video heartbeats record activity but skip evaluation; a derived auto-enrol is not counted as activity so replay is exact; the icon component is server-only; revocation is final for the rules and an admin re-awards. Founder decision mid-implementation: no backfill, since there are no users. Founder answered the open points on 2026-09-06; founder answered the open points on 2026-09-06 (decisions 7, 4, 5, 6 confirmed; decision 8 reversed: no CI failure, and `first_project_accepted` stays earnable through automated checks; notification limits added as decision 13; leaderboards revisitable). Spec entry: `docs/spec.md` → S8. Requirements: F1.16, F1.17, F1.13 (streaks and activity computed from the event stream); X5 (badges on the public profile); X10. Builds on S1 (`progress_events`, the reducer and `rebuildLearner`), S2 (`badges/*.yaml` already has a schema and two definitions in the content repo), S5 (notifications, admin role), S7 (public profile with a slot waiting for badges).
...
6. **No backfill.** Founder decision 2026-09-06, mid-implementation: the MVP has no users, so awarding badges retroactively is work with nothing to award. A badge published now applies to activity from now on. When there are learners and a badge is added, a backfill that replays each learner's stream through `evaluateBadges` is a short script (the evaluator is already idempotent and takes the whole event); it is on the backlog rather than in this spec. Manual admin awards cover the rare case in the meantime.
...
- `@repo/learning`: `evaluateBadges`, `awardBadge`, `recordActivity`, `streakFor`, `activityFor`.
...
- [x] Admins can award and revoke a badge manually, with a reason, recorded on the award row and shown in `/admin/badges` (decision 7 replaces the "existing audit trail" wording; founder to confirm).
- [x] Streak and activity graph computed from events and shown on the dashboard and the public profile.
- [x] `notify()` throttles emails per learner (4 an hour, 12 a day) without dropping the in-app row.
- [x] Browser loop passed on the dev server for the flows in test item 5; at least two fix-and-reload iterations recorded in `test.md`.

...
8. **`first_project_accepted` is rejected at content-check time.** Project review is deferred past the MVP (founder, 2026-09-06), so a badge with that rule can never award. `content:check` fails on it with a message pointing at the decision, rather than letting an author ship a badge nobody can earn. The rule kind stays in the schema for when projects return.
...
- `notification_kind` gains `badge_awarded`.
...
- `@repo/learning`: `evaluateBadges`, `awardBadge`, `recordActivity`, `streakFor`, `activityFor`, and `bin/backfill-badges.ts`.
...
- `badges/*.yaml`: the two existing badges stay; add `course-finisher` (`course_completed`, the seeded course) and `first-quiz` if a matching rule kind is added, so the loop has something to earn quickly. Any badge using `first_project_accepted` fails `content:check` (decision 8).
- `CONTRIBUTING.md` gains a badge section: the rule kinds, the icon rule, and that badges award retroactively.

## Tests

1. `@repo/learning` (Postgres): a `lessons_in_window` badge awards exactly once on the fifth lesson within the window and not on the sixth; a lesson outside the window does not award; `course_completed` and `path_completed` award from the derived events; `rebuildLearner` leaves awards and rebuilds activity exactly (no double counting); the backfill awards only what is missing; `streakFor` across a gap, across midnight in Karachi, and with today versus yesterday as the last active day; `activityFor` buckets by Karachi day.
2. `@repo/content` and `@repo/content-schema`: badges sync (create, update, archive) with counts in the summary; `content:check` rejects `first_project_accepted` and an unknown lucide icon.
3. `@repo/api` (Postgres): `catalogue` shows earned state and never leaks another learner's awards; `award` and `revoke` need admin and a reason and write provenance; a revoked badge disappears from the profile and the catalogue's earned state; `profiles.byHandle` includes badges and streak only for a public profile.
...
Badge tiers or points, leaderboards (deliberately: the platform is not a competition), notifications by email for badges, streak freezes or reminders, weekly digest (X4), sharing a badge as an image, `first_project_accepted` awards (returns with projects).
...
1. Manual awards record provenance on `user_badges` and appear in `/admin/badges`, rather than opening a moderation queue item the way a revoked certificate does (decision 7). Confirm.
2. Days are Asia/Karachi for everyone, and a streak survives if the last active day is today or yesterday (decisions 4 and 5). Confirm.
3. Badges award retroactively when a new one is added (decision 6), so publishing a badge can hand out hundreds of notifications at once. Confirm, or ask for retroactive awards to be silent (awarded without notifying).
4. No leaderboards or points at any stage, or only for now?

...
- F1.5 Exercises run in the browser: JavaScript and TypeScript only, through an in-house vitest-subset harness in a Web Worker (Sandpack was dropped in S4 as unmaintained), edited in CodeMirror. Founder decision at the end of S4: no other exercise languages (the Pyodide Python runner was removed); the platform does one runner well. Tests ship with the exercise and are shown to the learner; pass/fail is computed client-side and recorded server-side with the submitted code and results. Trust note (S4): the server never executes learner code, so a recorded pass is a self-report; the verified signal for employers is the reviewed project (area 1, F1.15) and certificate criteria (F1.18) can require one. Server-side verification is an open follow-up in `spec.md`.
...
- X8 **Performance budget** as in N1.1: a 250 KB first-load JS target per page, ratified as guidance rather than a gate (founder, 2026-09-12), with a 300 KB ceiling enforced by `pnpm check-budget` against a running production server. Progress and comments are dynamic islands. **The company bank and the catalogue are not statically rendered and do not use ISR**, which this requirement originally called for: a cached read layer was built in S10a and removed, because a review a moderator had just hidden stayed public for as long as the cache lived. Both are `force-dynamic` and read fresh on every request; they are cheap because they are indexed selects and a plain view. Caching returns only behind an invalidation path that a moderation decision can trigger, which is a spec of its own.

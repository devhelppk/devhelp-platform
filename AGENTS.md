...
| `packages/exercise-runner`                   | `@repo/exercise-runner` | In-house vitest-subset harness + Sucrase JS/TS runner and a Pyodide Python runner, each in a Web Worker in the browser; `@repo/exercise-runner/node` runs the same JS harness in Node for `content:check` parity. |
